...
std::vector<GroundedAction> TaskPlanner::run(Heuristic heuristicOption)
{
    ...
    while(task_motion_output.plan_status == PlannerStatus::REPLAN)
    {
        ...
        // remove not block
        std::list<std::string> failed_objects = actions[task_motion_output.fail_step_index].get_arg_values();
...
std::vector<GroundedAction> TaskPlanner::aStar(const GroundedConditionSet& startConditionSet,
                                               ...
                                               const bool& print)
{
    ...
    Node* currentNode = new Node(startConditionSet, 0, 0, nullptr, (*allPossibleActions.begin()));
    ...
    while (!open.empty())
    {
        ...
        for (auto action:allPossibleActions)
        {
            ...
            {
                GroundedConditionSet newConditions = applyAction(action, currentCond); //current cond will change here
                ...
                Node* newNode = new Node(newConditions, new_g, new_f, currentNode, action);
...
int TaskPlanner::getHeuristic(const Heuristic& heuristicOption, const GroundedConditionSet& goalSet, const GroundedConditionSet& currentSet,
                 const list<GroundedAction>& allPossibleActions)
{
    int h = 0;
    switch (heuristicOption)
    {
        ...
        {
            auto actions = aStar(currentSet, goalSet, allPossibleActions, NoHeuristic, false, false);
            h += (int)actions.size();
            break;
        }
        default:
